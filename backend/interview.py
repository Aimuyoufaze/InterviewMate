...
class InterviewSession:
    def __init__(self, persona_id: str, field: str):
...
async def start_interview(persona_id: str, field: str, background: str | None = None, resume: str | None = None) -> dict:
    ...
    msgs = [
        ...
        {"role": "user", "content": "请开始面试，先让我自我介绍。"}
    ]
...
async def continue_interview(session_id: str, user_message: str) -> dict:
    ...
    style_prompt = get_style_prompt(session.persona_id)
...
async def end_interview(session_id: str) -> dict:
    ...
    msgs = [
        {"role": "system", "content": system_msg},
        {"role": "user", "content": f"以下是本次模拟面试的记录：\n{transcript}\n请生成评估报告。"}
    ]
